...
[![Build Status](https://drone.osshelp.ru/api/badges/ansible/mysqld-exporter/status.svg)](https://drone.osshelp.ru/ansible/mysqld-exporter)

Role for mysqld_exporer installation and configuration.

## Usage (example)

```yaml
    - role: mysql
      mysql_no_restart: false
      users:
        - name: exporter
          password: exporterpass
          priv: "*.*:PROCESS,REPLICATION CLIENT"
    - role: mysqld-exporter
      mysqld_collector_flags:
        - {key: collect.perf_schema.eventsstatementssum, value: ""}
        - {key: collect.perf_schema.eventsstatements.limit, value: 250}
      mysqld_exporter_mycnf_params:
        user: exporter
        password: exporterpass
        host: localhost
...
| -------- | -------- | -------- |
| `mysqld_exporter_setup` | `full` | Setup mode. See [OSSHelp KB article](https://oss.help/kb4895) |
| `mysqld_collector_flags` | `[]` | List of mysqld_exporter collector flags |
...
- [Our article](https://placeholder)
- [Another link](https://placeholder)
